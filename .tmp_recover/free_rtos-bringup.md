# FreeRTOS bring-up ΓÇö Controls PCB (STM32G474)
...
| Signal | Meaning |
|--------|---------|
| PC2 ├ù3 boot flashes | `main()` alive through early GPIO |
| PC2 solid ON | `CDC_Init_FS` ΓÇö USB enumerated and configured by host |
| PC3 ~1 Hz toggle | TIM6 ISR running (`control_loop_tick`) |
| PC1 HIGH ΓåÆ LOW | Bring-up milestone: USB init returned ΓåÆ `app_init()` returned |
| CAN CH1ΓÇô6 LEDs solid | `can_router_init()` finished (pre-scheduler; **not** proof RTOS runs) |
...
| Commit | Label |
|--------|--------|
| `14eb426` | Last **pre-RTOS** commit ΓÇö Damiao enable working, USB + heartbeat nominal |
| `cfecd2e` | First RTOS commit (ΓÇ£mid-migrationΓÇ¥) ΓÇö kernel integration; regressions begin |
...
| Item | Value |
|------|--------|
| RTOS API | **CMSIS-RTOS v1** (`cmsis_os.h` / `osThreadCreate`) ΓÇö not v2 |
| Kernel | FreeRTOS 10.3.1 (Cube FW_G4 1.6.3) |
| HAL timebase | **TIM7** @ 1 ms (`VP_SYS_VS_tim7` in `.ioc`) |
| Kernel tick | **SysTick** ΓåÆ `xPortSysTickHandler()` (FreeRTOS owns SysTick) |
| Heap | Dynamic only, `configTOTAL_HEAP_SIZE = 24 KiB` |
| FPU | `configENABLE_FPU = 0` (G474 has FPU; disabled for port simplicity) |
| `configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY` | **5** (ST UM1722 example; ISRs that call `FromISR` APIs must be priority **ΓëÑ 6**) |
...
```
HAL_Init ΓåÆ clock ΓåÆ GPIO (+ boot pulses) ΓåÆ FDCAN/SPI/UART/TIM6
ΓåÆ control_loop_start()          // arms TIM6 + first HB edge
ΓåÆ MX_USB_Device_Init()          // BEFORE blocking plant init
ΓåÆ app_init()                    // MCP2518 SPI, mutexes, host_link, ΓÇª
ΓåÆ MX_FREERTOS_Init()
ΓåÆ HAL_SuspendTick()             // stop TIM7 HAL tick during scheduler handoff
ΓåÆ osKernelStart()               // never returns
```
...
|------|----------|---------------|------|
| **ControlTask** | `osPriorityAboveNormal` (4) | 512 | Drains `g_control_ticks_pending` via `control_loop_service()` ΓÇö actuator apply/capture + servo |
| **StartDefaultTask** (host link) | `osPriorityNormal` (2) | 1024 | `HAL_ResumeTick()` then `app_run()` + `osDelay(1)` ΓÇö USB host link, diag, LEDs |
...
### Control loop ΓÇö ISR vs task

**Pre-RTOS (`14eb426`):**

- TIM6 ISR (`control_loop_tick`): increment counter, toggle PC3, bump `g_control_ticks_pending`
- Main superloop (`app_run` ΓåÆ `control_loop_service`): drain pending ticks, run actuator/servo

**Post-RTOS (resolved configuration):**

- Same ISR pattern ΓÇö no FreeRTOS calls inside TIM6
...
| Area | Change |
|------|--------|
| `can_router.c` | Per-FDCAN-bus `xSemaphoreCreateMutex()` (CH1ΓÇô3 independent) |
| `spi_can_port.c` | One SPI1 mutex for CH4ΓÇô6 MCP2518 rails (replaces `__disable_irq()` spinlock ΓÇö deadlock hazard under preemption) |
...
- Empty `SVC_Handler` / `PendSV_Handler` / `SysTick_Handler` stubs removed from `stm32g4xx_it.c` ΓÇö port maps `vPortSVCHandler` ΓåÆ `SVC_Handler`, `xPortPendSVHandler` ΓåÆ `PendSV_Handler`
- `SysTick_Handler()` calls `xPortSysTickHandler()` only when scheduler Γëá `NOT_STARTED`
- `HAL_TIM_PeriodElapsedCallback` in `main.c` merged: TIM6 ΓåÆ `control_loop_tick()`, TIM7 ΓåÆ `HAL_IncTick()` (Cube regeneration may split these ΓÇö re-merge required)
...
| UART4 | 0 | 6 | 6 (TELEM mode ΓÇö inert unless `HOST_TRANSPORT_UART`) |
| MCP2518 EXTI | ΓÇö | 6 | 6 |
| TIM7 (HAL tick) | ΓÇö | 15 | 15 |
...
### Phase 0 ΓÇö first RTOS commit (`cfecd2e`)
...
### Phase 1ΓÇô2 ΓÇö task split + notify-from-ISR
...
### Phase 3 ΓÇö decouple USB/heartbeat from scheduler

- `control_loop_start()` + `MX_USB_Device_Init()` moved back to `main()`
- UART4 `DAMIAO_BRIDGE` ΓåÆ `TELEM` to eliminate spurious UART4 IRQ during bench
- PC3 remained solid; USB still failed ΓÇö ruled out ΓÇ£USB inside a task that never runsΓÇ¥ as sole cause

### Phase 4 ΓÇö init order + diagnostic milestones
...
### Phase 5 ΓÇö root cause resolution
...
## Identified issues ΓÇö ranked by impact

### 1. USB NVIC priority 0 ΓåÆ 6 (Code 43)

| | |
|---|---|
| **Configuration** | USB_LP at priority 6 while `app_init()` blocks for seconds in MCP2518 SPI + `HAL_Delay` |
| **Mechanism** | Host begins enumeration immediately after `MX_USB_Device_Init()`. Descriptor reads overlap `mcp2518_init_all()`. At priority 6, USB LP IRQ does not reliably preempt that work ΓåÆ SETUP/DESCRIPTOR timeout ΓåÆ **Code 43** |
| **Resolution** | `HAL_NVIC_SetPriority(USB_LP_IRQn, 0, 0)` in `USB_Device/Target/usbd_conf.c` |
| **Note** | RTOS syscall priority ceiling applies only to ISRs that call `FromISR` APIs ΓÇö not to USB or GPIO heartbeat |

### 2. FreeRTOS calls inside TIM6 ISR (solid heartbeat)

| | |
|---|---|
| **Configuration** | `vTaskNotifyGiveFromISR` in `control_loop_tick()`; TIM6 at priority ΓëÑ 6 |
...
### 3. False indicator: ΓÇ£CAN LEDs solid = RTOS worksΓÇ¥

| | |
|---|---|
| **Observation** | Solid CAN activity LEDs after boot |
| **Actual meaning** | `can_router_init()` runs in `main()` before `osKernelStart()` ΓÇö does not confirm task execution |
...
`configMAX_SYSCALL_INTERRUPT_PRIORITY = 5` masks IRQs with NVIC priority **ΓëÑ 5**. TIM6 at priority 6 is masked during critical sections. Acceptable when critical sections are short; additional reason to keep TIM6 at priority 0 for bring-up signals.

### 7. CubeMX regeneration hazards

- Duplicate `HAL_TIM_PeriodElapsedCallback` ΓÇö merge TIM6 + TIM7 into USER CODE 0
- `.ioc` may reset TIM6/USB NVIC to 6 ΓÇö verify `tim.c` / `usbd_conf.c` after regeneration
...
### Level 0 ΓÇö Pre-scheduler (power-on, no host tools)

After flash, wait **2ΓÇô3 seconds**:

| Check | Pass criterion |
|-------|----------------|
| PC2 ├ù3 boot flashes | `main()` early path |
...
| CAN CH1ΓÇô6 LEDs solid | Router init complete |

PC3 blink plus PC2 solid indicates **pre-RTOS parity** before task-level checks.

### Level 1 ΓÇö USB host link (confirms `StartDefaultTask` + scheduler)

**Important:** COM port enumeration only proves `CDC_Init_FS` ran. `link-test` requires
`StartDefaultTask` ΓåÆ `app_run()` ΓåÆ `host_link_poll_tx()`. If link-test times out with
no 562 B feedback, the scheduler is not running tasks (common cause: broken SysTick wiring
ΓÇö `xPortSysTickHandler` must map to `SysTick_Handler` in `FreeRTOSConfig.h` with no
...
A passing `link-test` confirms **`StartDefaultTask` is running** under the scheduler (`app_run` ΓåÆ `host_link_poll_*`).

### Level 2 ΓÇö Control loop under RTOS (confirms `ControlTask`)

Indirect checks without JTAG:

1. `link-test` and `status` remain reliable during CH1/CH3 motor activity ΓÇö host and control tasks coexist.
2. RobStride probe/teleop on CH1 ΓÇö sustained 500 Hz command path requires `host_link_command_is_fresh()` and ControlTask draining ticks:
...
### Level 3 ΓÇö Debugger (optional)

Breakpoints in CubeIDE:

1. First line of `ControlTask` ΓÇö should hit after `osKernelStart`
2. First line of `StartDefaultTask` ΓÇö should hit after `osKernelStart`
3. `control_loop_service()` ΓÇö regular hits at ~1 kHz poll rate (`osDelay(1)` batches tick bursts)

If (2) hits but (3) does not, inspect ControlTask creation or priority assignment.

### Level 4 ΓÇö `configASSERT` visibility

`FreeRTOSConfig.h` maps `configASSERT` ΓåÆ `Error_Handler()` (PC2+PC3 slow blink). A silent `for(;;)` with IRQs disabled obscures assert trips during bring-up.

---

## Host tool: `controls_pcb_host` ΓÇö Damiao enable on CH3

Package: `scripts/controls_pcb_host/` (CLI: `scripts/controls_pcb_host.py`).

Firmware: slot **2** = Damiao, `CAN_BUS_CH3`, `PROTO_DAMIAO` (`plant_config.c`). CH3 = **`hfdcan2`** standard CAN @ 1 Mbps ΓÇö see [damiao-bringup.md](damiao-bringup.md) for termination and register map.
...
1. `dm_session_begin` ΓÇö firmware `mcu_state = DIAG_ONLY`, CH3 dedicated to DM0 probes
...
5. `dm_session_end` ΓÇö return toward normal plant mode

**Pass criteria:** console prints `OK: motor reports enabled (ERR=0x1).` and/or `format_hit` with plausible `pos` / `err`.

**Electrical:** `tx>0` with `rx_raw=0` indicates a physical-layer issue (typically **120 ╬⌐ termination at motor end**), not RTOS ΓÇö see [damiao-bringup.md](damiao-bringup.md).
...
## Definition of done ΓÇö FreeRTOS path

```
[ ] PC3 heartbeat ~1 Hz with USB cable connected
[ ] Windows COM port (no Code 43) ΓÇö CDC stable after 3 s from power-on
[ ] controls_pcb_host link-test ΓåÆ exit 0
[ ] controls_pcb_host status ΓåÆ valid feedback header
[ ] CH1 RobStride probe still works (slot 0)
[ ] Damiao discover OR probe on CH3 (RX depends on termination, not RTOS)
[ ] Damiao --enable ΓåÆ ERR=0x1 (when motor + bus are verified)
...
- [bringup.md](bringup.md) ΓÇö transport selection, motor map, legacy `damiao_scan.py` commands
- [damiao-bringup.md](damiao-bringup.md) ΓÇö CH3 termination, register map, symptom tree
- [architecture.md](architecture.md) ΓÇö module map
- `RTOS_BRINGUP_HANDOFF.txt` ΓÇö detailed round-by-round debug log
