...
|-------|----------------------|---------------|
| Controls PCB (laptop) | `0` | USB FS CDC ΓåÆ `COM*` / `/dev/ttyACM*` |
...
|------|-----|----------|----------|
| 0ΓÇô6 | CH1 | `0x01`ΓÇª`0x07` (factory placeholders) | Damiao |
...
- **CH3:** **mixed** standard + extended classic CAN ΓÇö `hfdcan2` @ PB12/PB13 (`StdFiltersNbr=1`, `ExtFiltersNbr=1`, accept-all dual filter). Damiao (std) and RobStride (ext) may share the branch when configured on different actuator slots. See [fdcan-dual-id-mixed-bus.md](fdcan-dual-id-mixed-bus.md).
- `can_router.c` maps schematic CH2 ΓåÆ `hfdcan3` (PA8/PA15), CH3 ΓåÆ `hfdcan2` (PB12/PB13)
- Activity LEDs: PC7 (CH1), PC6 (CH2), PB15 (CH3)
- **CH4ΓÇôCH6:** MCP2518FD SPI-CAN ΓÇö see [ch4-mcp2518-bringup-postmortem.md](ch4-mcp2518-bringup-postmortem.md)
...
### Damiao CH1 daisy chain (in progress ΓÇö Jul 2026)
...
|-------|----------------|
| USB / DM0 session | OK |
| Discover **DM-J4310** | **OK** ΓÇö register scan finds 4310 ESC_IDs on CH1 |
| Discover **DM-J4340P-2EC** (isolated CH1) | **OK** ΓÇö same MIT + `0x7FF` reg-scan as 4310 (docs register map / read-param identical). Bench unit: `ESC_ID=0x01`, `Master=0x11`. Enable (`0xFC`) + MIT feedback OK (`ERR=1`, pos/temp). |
| Plant slot map (RAM) | Slot 0 ΓåÆ Damiao CH1 `0x01`; other slots disabled for isolate test. **`--persist` flash SAVE currently `flash_err`** (RAM survives until power cycle; needs NVM/flash fix + reflash). |
| Plant feedback (slot 0) | **OK** ΓÇö `status` shows `slot0 posΓëê-0.75 err=0x1` while motor enabled |
| Plant teleop jog | **Pending** ΓÇö run `teleop --slot 0` interactively |
| Multi-motor daisy teleop | **Not yet** ΓÇö previously 4310s faulted when un-enabled 4340s sat mid-chain; re-test after each 4340 ID is mapped+enabled |

**Doc note:** Extended manuals (`DMJ4310 Documentation Extended.pdf` vs `DM-J4340P-2EC V1.1 Documentation Extended.pdf`) share the same CAN 2.0B @ 1 Mbps MIT / `0x7FF` param protocol and register addresses. No separate `PROTO_DAMIAO_4340`. If a unit never answers, check Assistant baud (codes >1M ΓåÆ CAN FD feedback the MCU will not decode) and ESC_ID range ΓÇö not a different wire format.
...
Discovery uses **register read** (`DM_PROBE_REG_SCAN`): TX `0x7FF` read ESC_ID (`0x08`) + MST_ID (`0x07`). Works while motor is **disabled** (red LED). Map each FOUND `esc_id` / `master_rx` to a plant slot with `config set --bus 1 --motor-id ΓÇª` (add `--persist` once flash SAVE works).
...
| YAM 7-motor daisy | 0ΓÇô6 | CH1 | TBD after discover | | `ACTUATOR_COUNT=7` ΓÇö reflash required; then `damiao_scan --discover --host-only --bus 1` |
...
ESC_ID/Master column is the nominal factory-placeholder mapping (`0x01`ΓÇª`0x07` /
`0x11`ΓÇª`0x17`); confirm against the actual daisy chain with `damiao_scan --discover`
(see the isolate-bench map above) before trusting slot assignment. J1ΓÇôJ6 ranges come
...
| AI (single-joint, non-interactive) | `hello-world --joint N ΓÇª` / `joint goto --joint N ΓÇª` / `joint status --joint N` |
...
`joint goto --to --absolute` is refused without `--i-know-zeros` ΓÇö motor encoder frame
is not yet aligned to the MuJoCo model frame (calibration steps in the plan ┬º6).
...
soft limits and gains exactly (identical hardware) ΓÇö see `arm_local_joint()` /
...
rebuild + reflash ΓÇö see git history on `App/`):

| Bug | Symptom | Fix |
|-----|---------|-----|
| CFG PDU packing overflowed at 14 slots | `_Static_assert` build failure (6+14├ù3 > 32 B PDU) | Paginated CFG GET: firmware replies Γëñ8 slots/page + `[total_count][start_slot]` trailer; host (`plugins/plant_config.py::fetch_table`) loops pages automatically. `plant_config_nvm.c` |
| Damiao TX tripled per slot per tick | J6/J7 (last-enqueued slots) silently stopped tracking MIT commands once 6-7 Damiao slots were enabled at once ΓÇö CH1 bus oversubscribed (~10.5k frames/sec demand vs ~7.7-9.3k capacity) | `damiao_apply_cycle()`: 3├ù redundant MIT send ΓåÆ 1├ù. `plugins/damiao.c` |
...
- **No more gravity sag on release** ΓÇö `_update_slot_motion` used to drop to a weak
...
- **Non-target joints no longer go limp** ΓÇö a plain single-slot `send_plant({slot: ...})`
...
- **Home to current position, not a fixed 0 rad** ΓÇö `_home()` takes a per-slot
...
- **Group selection by joint number across arms** ΓÇö `--plant-teleop --joints 1,2,3,4`
  ...
  `--joints 1,2,3,4` ΓåÆ slots `0,1,2,3,7,8,9,10`. `--plant-slots` (raw slot list) still
  works for asymmetric picks. Bus-select keys (`0`/`1`/`2`) now usefully split by arm
  since arm2 is on its own bus ΓÇö `1`=arm1 only, `2`=arm2 only, `0`=both together.

**Gain/velocity tuning history** (bench feedback, Jul 2026 ΓÇö see `teleop/defaults.py`):

| Setting | History | Current |
|---|---|---|
| `SLOT_KP` (per-joint) | flat 12.0 ΓåÆ (30,50,90,50,12,12,12) ΓåÆ (40,60,90,60,25,25,20), mirrored per arm | wrist/EE joints bumped once they started holding at full stiffness continuously (weren't firm enough for that duty at 12.0) |
| `DM_KD` | 0.5 ΓåÆ 0.8 ΓåÆ 1.0 | more damping to match higher `SLOT_KP` |
| `DM_ARROW_VEL` | 3.0 ΓåÆ 0.6 ΓåÆ 0.25 ΓåÆ 0.12 rad/s | still "a little high" per bench feedback ΓÇö next lever to pull if whip persists |
| `MAX_CMD_LEAD` | 0.35 ΓåÆ 0.18 rad | caps how far cmd can run ahead of fb before capping ΓÇö was contributing to "whipping" (stored torque released suddenly) |
...
### Damiao CH3 (earlier bench ΓÇö Jul 2026)

Single DM-J4310 on CH3: USB/DM0 OK, CAN TX OK; **`rx_raw=0`** on register scan until motor-end **120 ╬⌐** termination is added (4310 has no software termination register). Superseded for multi-motor work by CH1 daisy-chain bench above.
...
**Architecture (FIFOs, filter masks, demux):** [fdcan-dual-id-mixed-bus.md](fdcan-dual-id-mixed-bus.md) ┬º0 ΓÇö HW RX FIFO0 is shared; std vs ext is identified by HAL `IdType` / `can_frame_t.id_type`; software fan-out replaces per-plugin ring drains.

**Bench verified (Jul 2026):** four-slot plant teleop `0,1,2,3` ΓÇö Damiao + RS on CH1ΓÇôCH3, including Damiao + RS both on CH1. Smooth arrow motion after homing when all configured motors are enabled. **Damiao-only CH1 daisy chain:** homing OK on DM-J4310 slots; teleop still blocked when un-enabled DM-J4340 units sit between configured 4310s ΓÇö see ┬º Damiao CH1 daisy chain.
...
1. `discover --protocol damiao --bus 3` ΓåÆ FOUND (std frames).
2. `discover --protocol robstride --bus 3` ΓåÆ FOUND when RS motor on CH3 (ext frames).
...
All devices on a mixed branch must share **1 Mbps** nominal bit timing and proper **120 ╬⌐** termination at each end.
...
### Plant teleop (`--plant-teleop`) ΓÇö recommended runtime path
...
- Gentle defaults: kp 8ΓÇô12 (gated ΓÇö **0 at rest**), 5 rad/s, slow ramps ΓåÆ low bench current
...
Sequential capability demo with **15% stagger** on CH1 daisy chain (`0x76 ΓåÆ 0x74`).
...
### RS2 PDU path ΓÇö calibrate, discover, single-motor teleop

Uses `pdu.data[0..2] = 'R','S','2'` and `pdu.data[11]` for schematic bus (`1` = CH1 ΓÇª `3` = CH3). Pauses the 500 Hz actuator loop while an RS2 session is active.
...
Slot 0 = ID 1 (bottom), slot 1 = ID 2 (top). Unicast wr/rd @ 500 Hz on MCU; host @ ~40 Hz. See **Closed ΓÇö Dynamixel neck** in [known-issues.md](known-issues.md).
...
5 V on strip, GND common with MCU. Mode 0 = red dot scan from input end. Tune `LED_STRIP_MAX` in `App/Inc/plant/plugins/sk9822.h`. See **Closed ΓÇö SK9822** in [known-issues.md](known-issues.md).
...
| All RobStride slots | Feedback populates after wake; homing completes ΓåÆ arrow keys enabled |
...
| Cal reports **NOISE** | Daisy-chain bus issue ΓÇö power-cycle drives, retry (see known-issues) |
| MCU stuck after Ctrl+C mid-probe | Short `0x70` reset or `--recovery` on affected bus |
| Wrong bus / LED | CH2/CH3 Cube instance swap ΓÇö use schematic bus in scripts, not Cube name |
| Damiao `tx>0` `rx_raw=0` | Missing motor-end 120 ╬⌐ termination; 24 V; see [known-issues.md](known-issues.md) |
| Damiao 4310 teleop fault mid-chain | Un-enabled DM-J4340 between configured 4310 slots ΓÇö discover/enable 4340 or re-chain |
| Garbage / no apply | USB port mismatch, or RX desync (magic hunt in `host_link`) |

## 7. Plant teleop cadence (FDCAN + MCP) ΓÇö **resolved Jul 2026**

Regression after commits **`d9ce9e6`** / **`c700c78`** (last known-good CH2: **`5df1f04`**). Restoring `CONTROL_TICK_BURST_MAX=8` and a single `control_loop_service()` per lap was **necessary but not sufficient** ΓÇö several independent bugs stacked on top of the MCP superloop work.

### Symptom (failing builds)

- `cmd` updates smoothly but `fb` jumps in lumps; motion feels stepped.
- Telemetry: `pend` pegged, `lap` **~50ΓÇô370 ms**, `ptick` Γë¬ burst max, `lead` often pinned at **┬▒0.35 rad**.
...
| Layer | Cause | Fix |
|-------|--------|-----|
| **Superloop** | `d9ce9e6`/`c700c78`: burst 1, 4├ù service loop, all 6 slots + full `can_router_poll()` every tick | Restore burst **8**, single `control_loop_service()`; scope polls to commanded buses; skip blank MCP slots |
| **Dynamixel** | `servo_bus_service()` ran every plant burst with no servo host session ΓåÆ UART RX **~50 ms** timeout ΓåÆ `lapΓëê52 ms`, `pend=255` on **all** plant teleop (including CH2) | Skip DXL bus unless `g_servo_host_session` |
| **MCP init** | Lazy MCP rail init (one rail per `spi_can_router_hw_step`) never finished once end-of-lap stopped walking all SPI rails | **Eager** `mcp2518_reinit_rail()` for CH4ΓÇô6 in `spi_can_router_init()` |
| **MCP plant TX** | `enqueue` + `try_send` with 3 ms TXQ wait ├ù burst 8 ΓåÆ `lapΓëê327 ms`; blocking `probe_tx` (50 ms) same problem | Plant MIT: `enqueue` + `prepare_tx` + **fire-and-forget** `mcp2518_try_send` (load TXQ, no wait). Probes/recovery still use blocking `send_now` |
...
| **Recovery LEDs** | `recover` runs `plant_recovery_all()` on **all** enabled RS02 slots (CH1ΓÇô3 FDCAN + CH4ΓÇô6 MCP), not only `--bus N` | Expected: CH1ΓÇô2 blink on any recover; MCP reset uses `send_now` so CH4 blinks when rail is initΓÇÖd |
| **Host teleop** | `FB_STALE` gate treated flat `fb` as dead ΓåÆ `kp=0` with arrow held; blank MCP desire skipped firmware SPI | Gate on **ack** age only; active slot sends `HOME_POS_EPS` so MCP is non-blank |
| **Host cmd slew** | `cmd = fb + lead` each tick rebased onto lagged `fb` ΓåÆ visible snaps | Integrate `cmd` from previous `cmd`; clamp lead widening only |
| **`fb_age` metric** | Host only reset timer when position moved **>1e-4 rad** ΓÇö flat rest looked ΓÇ£staleΓÇ¥ during motion | Reset `fb_age` on **any** fresh actuator sample for the slot |

**Discrepancy note:** Early traces showed `lapΓëê52 ms` with `pend=255` and were attributed to CAN/MCP polling alone. Profiling showed **`lap` matched `DXL_RX_TIMEOUT_MS` (50)** ΓÇö Dynamixel was the dominant cost until the servo skip. After that, CH2 reached `lapΓëê0ΓÇô1 ms`. MCP then failed for different reasons (init, host gating, blocking TX), not superloop burst settings.

### Good telemetry (post-fix, reflash required)

| Bus | `lap` | `pend` | `fb_age` (motion) | Notes |
|-----|-------|--------|-------------------|-------|
| CH2 FDCAN slot 1 | 0ΓÇô1 ms | 0 | 0 ms | Smooth `cmd`/`fb` tracking |
| CH4 MCP slot 3 | 0ΓÇô1 ms | 0ΓÇô1 | 0 ms | CH4 ACT LED blinks during arrow hold |

At rest, `fb_age` may climb while `fb` is flat ΓÇö that means position has not changed, not that USB feedback stopped.
...
- `dir` briefly `0` while arrow held ΓåÆ host latch (`RELEASE_CONFIRM_S` in `teleop/input.py`). Separate from plant cadence.
...
## 8. CH2 cali after teleop ΓÇö **resolved Jul 2026**
...
Expect `... cali listen` lines and shaft spin. If cali fails cold (no teleop), check harness/termination ΓÇö not this handoff path.
