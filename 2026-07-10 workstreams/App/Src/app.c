...
void app_init(void)
{
	...
	led_init();
	plant_config_init();
	thermo_init();   /* 2026-07-10 workstreams — MAX31855 bench thermocouple */
...
void app_run(void)
{
	...
	led_service();
	/* 2026-07-10 workstreams: low-rate (~5 Hz, self-gated internally), same
	 * shape as led_service() — one blocking SPI transaction per call, never
	 * on the 500 Hz control_loop_service() path. */
	thermo_service();
