#!/usr/bin/env python3
"""controls_hub_api usage examples - PLANT + HEALTH modes, no GUI.

    python scripts/controls_hub_api_example.py hold --port COM5 --slot 3 --kp 8 --kd 0.45
    python scripts/controls_hub_api_example.py sine --port COM5 --slot 3 --hz 100
"""
...
from controls_hub_api import ActuatorDesire, ControlsHub  # noqa: E402


def cmd_hold(args: argparse.Namespace) -> None:
    """Example A: single MIT hold, printing hub.health snapshots."""
    with ControlsHub.connect(args.port) as hub:
        hub.plant.recover()
        hub.plant.set_actuator(
        ...
        while time.monotonic() < deadline:
            fb = hub.plant.read_feedback(timeout_s=1.0)
            act = fb.actuator(args.slot)
            health = hub.health.snapshot()
            print(
                f"tick={health.tick} ack={health.ack_seq} plant_block={health.plant_block} "
                f"pos={act.position:+.4f} vel={act.velocity:+.4f}"
            )
            hub.plant.send_once()  # keep under the 500 ms host_stale watchdog
            time.sleep(0.1)
...
def cmd_sine(args: argparse.Namespace) -> None:
    ...
    with ControlsHub.connect(args.port) as hub:
        ...
        hub.plant.run_at_hz(args.hz, trajectory, stop=lambda: time.perf_counter() >= stop_at)
