#!/usr/bin/env python3
"""One-shot remote continuous prove: sync stack, cruise, peer soft-kill.

Does not sync debug_dashboard (GUI lane). Prefer Soft-kill Park in follow mode
when a dashboard is already open on the Jetson.
"""
...
LOCAL_LEGACY = Path(__file__).resolve().parent
LOCAL_SCRIPTS = LOCAL_LEGACY.parent
OUT = LOCAL_LEGACY / "_tmp_continuous_out.txt"

DURATION_S = 50.0
BOOT_WAIT_S = 55.0
SOFTKILL_AT_CRUISE_S = 22.0  # after RS should have reversed at MIT rail

# (local under scripts/, remote under REMOTE — remote stays flat for Jetson)
SYNC_FILES = (
    ("legacy/yam_continuous_all.py", "yam_continuous_all.py"),
    ("legacy/pdb_uart_sim.py", "pdb_uart_sim.py"),
    ("legacy/stop_can.py", "stop_can.py"),
    ("legacy/rs02_channel_bringup.py", "rs02_channel_bringup.py"),
    ...
    ("deft_controls_sdk/bench/servo_fb.py", "deft_controls_sdk/bench/servo_fb.py"),
...
def main() -> int:
    ...
    for local_rel, remote_rel in SYNC_FILES:
        local = LOCAL_SCRIPTS / local_rel.replace("/", os.sep)
        if local.is_file():
            sftp.put(str(local), f"{REMOTE}/{remote_rel}")
    sftp.close()

    _, o, _ = c.exec_command(f"cd {REMOTE} && python3 -u stop_can.py", timeout=30)
    print(o.read().decode("utf-8", "replace"))
