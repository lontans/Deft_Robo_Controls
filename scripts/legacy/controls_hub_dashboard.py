...
LEGACY COM OWNER: this GUI opens the serial port directly (HubSession owns it for the
life of the process) rather than going through the ControlsHub façade
(scripts/controls_hub_api/). Do not run it against a port another session/process
already holds, and do not add new serial features here - see
docs/plan-host-api-streamline.md and docs/controls_hub_dashboard.md for the preferred
direction (client of a future hubd / read-only health, not a second serial owner).
