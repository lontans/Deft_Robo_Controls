...
from deft_controls_sdk.link.exchange.wire_layout import LED_CMD_OFF, PDB_OFF, SYSTEM_FB_OFF
from deft_controls_sdk.pdb.frame import KILL_STATE_NAMES, MAGIC_FB, parse_feedback as parse_pdbf
...
def _sys_kill(raw: bytes) -> tuple[int, int, int]:
...
def _sample(hub: ControlsPcbHub, seconds: float = 1.2) -> dict:
    ...
    ks, kr, es = _sys_kill(last)
    pdb = parse_pdbf(last[PDB_OFF : PDB_OFF + 64])
...
def main() -> int:
    ...
    try:
        ...
        with ControlsPcbHub.connect(args.port, persist_telemetry=False) as hub:
            ...
            time.sleep(1.0)
            s = _sample(hub, 1.2)
            ok_r = s["kill_state"] == 0 and s["led_mode"] == 3
            results.append(
