...
from deft_controls_sdk.link.exchange.wire_layout import (  # noqa: E402
    LED_CMD_OFF,
    PDB_OFF,
    SYSTEM_FB_OFF,
)
from deft_controls_sdk.pdb.frame import (  # noqa: E402
    KILL_STATE_NAMES,
    MAGIC_FB,
    parse_feedback as parse_pdbf,
)
...
def _wait_kill(
    ...
    while time.perf_counter() < deadline:
        hub.send_once()
        while True:
            raw = _conn(hub).reader.pop()
            if raw is None:
                break
            hdr = parse_feedback_header(raw)
            if hdr is None or hdr.get("is_debug"):
                continue
            last, _, _ = struct.unpack_from("<BBB", raw, SYSTEM_FB_OFF + 14)
            if int(last) == want:
    ...
    *,
    ...
    while time.perf_counter() < t_end:
        ...
        while True:
            ...
            ks, _kr, es = struct.unpack_from("<BBB", raw, SYSTEM_FB_OFF + 14)
            kill_states.append(int(ks))
            local_estops.append(int(es))

            pdb = parse_pdbf(raw[PDB_OFF : PDB_OFF + 64])
            if pdb is not None:
                pdb_samples.append(pdb)
