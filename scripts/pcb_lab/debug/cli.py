"""CLI: ``python -m pcb_lab.debug scan|show|set …``."""
...
from pcb_lab.debug.cfg_editor import run_cfg_editor
from pcb_lab.debug.proto import parse_protocol
from pcb_lab.debug.show import (
    collect_bandwidth,
    collect_cfg,
    collect_status,
    format_cfg_table,
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="pcb_lab.debug",
        description="Scan ports / show status / set CFG (NVM)",
    )
    ...
    sh.add_argument(
        "--bandwidth",
        action="store_true",
        help="host stream_hz + plant/periph lap timings",
    )
    sh.add_argument(
        "--status",
        action="store_true",
        help="HostProxy doctor (mcu / pdb / led / stream)",
    )
    sh.add_argument(
        "--pcb",
        action="store_true",
...
def _cmd_show(proxy: HostProxy, args: argparse.Namespace) -> int:
    ...
    want_st = bool(args.status)
    if want_pcb and not (want_cfg or want_bw or want_st):
        settle = max(0.0, float(args.settle_ms) / 1000.0)
        if settle:
            time.sleep(settle)
        from pcb_lab.debug.pcb_tui import run_pcb_dashboard
    ...
    if not (want_cfg or want_bw or want_st or want_pcb):
        # Default: everything — matches “show me the board”
        want_cfg = want_bw = want_st = True
    ...
    if want_st:
        payload["status"] = collect_status(proxy)
    ...
    if want_cfg:
        ...
        if want_bw or want_st or want_pcb:
            print()
