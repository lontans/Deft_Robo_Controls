"""Component CFG presets for ``pcb_lab.debug set --cfg`` -> ``preset``.
...
from deft_controls_sdk.link import LedDesire, ServoDesire
...
def apply_neck_preset(proxy, preset: NeckPreset, periph: dict) -> None:
    ...
    proxy.set_servo(
        preset.pitch_slot,
        ServoDesire(
            servo_id=preset.pitch_id,
            native_step_position=2048,
            torque_enable=True,
            operating_mode=3,
        ),
        send=False,
...
def apply_led_preset(proxy, preset: LedPreset, periph: dict) -> None:
    """Update NVM LED defaults + stage host LedDesire policy."""
    ...
    proxy.set_led(
        LedDesire(
            ...
            master_brightness=preset.brightness,
        ),
        send=True,
    )
