...
static FDCAN_HandleTypeDef *bus_handle[] = {
	...
	&hfdcan3, /* CAN_BUS_CH2: PA8/PA15 (sheet CH2, motor 0x72) — not Cube "FDCAN2" */
	&hfdcan2, /* CAN_BUS_CH3: PB12/PB13 (sheet CH3, motor 0x75) — not Cube "FDCAN3" */
};

static const can_act_led_t can_act_led[] = {
...

{
	...
	/* RS02 private protocol uses classic 8-byte data. Identifier is the proven

	 * bench signal; always pass through the full HAL rx buffer. */
...

{
	...
	for (int i = 0; i < (int)CAN_BACKEND_COUNT; i++) {
		...
		while (backend_recv(bus, &temp) == CAN_OK) {
			...
			/* Drop oldest on overflow — long cal listen must not lose the latest

			 * motor_feedback when proactive 0x18 from other daisy-chain nodes fills RX. */
