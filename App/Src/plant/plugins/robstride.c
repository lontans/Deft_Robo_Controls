                                          ...
                                          robstride_probe_result_t *out)
{
	...
	if (frame_in == NULL || out == NULL || frame_in->dlc < 8)
		return false;
	...
	if (expect_index != 0u) {
		uint16_t idx_echo = (uint16_t)frame_in->data[0] |
		                    ((uint16_t)frame_in->data[1] << 8u);
		if (idx_echo != expect_index)
			return false;
	}
	...
	out->position = robstride_bytes_to_float(frame_in->data);
	return true;
}
                                         ...
                                         robstride_probe_result_t *out)
{
	if (frame_in == NULL || out == NULL)
		return false;
	if (frame_in->id_type != CAN_ID_EXT || frame_in->dlc < 8)
		return false;
...
void robstride_apply_cycle(const actuator_config_t *cfg,
                           ...
                           actuator_state_t *state_out)
{
	static uint32_t last_maintain_ms;
	can_frame_t frame;
	can_bus_id_t bus;
	...
	bus = cfg->bus;
	robstride_maintain_enable(cfg, &last_maintain_ms);
	...
	while (can_rx_pop(bus, &frame) == CAN_OK) {
		if (state_out != NULL)
			(void)plugin_parse_rx(cfg, &frame, state_out);
	}
}
                        ...
                        robstride_probe_result_t *out)
{
	...
	const can_bus_id_t bus = CAN_BUS_CH1;
	...
	bool do_run_mode = !reset_only && !ctrl_fast && !para_read && !proactive && !bench_cmd &&
	                   (probe_kind <= RS02_PROBE_FULL || promiscuous || enable_only);
	bool do_enable = !reset_only && !ctrl_fast && !para_read && !proactive && !bench_cmd &&
	                 (probe_kind <= RS02_PROBE_ENABLE_CTRL || promiscuous || enable_only);
	bool do_ctrl = !reset_only && !enable_only && !para_read && !proactive && !bench_cmd;
	...
	if (proactive) {
		...
		if (robstride_send_proactive(&cfg, 1u, &frame) != PLUGIN_OK)
			return false;
		if (robstride_enqueue(bus, &frame) != PLUGIN_OK)
			return false;
		can_router_poll();
