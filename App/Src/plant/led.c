 ...
 *   NORMAL + fresh     → SOLID_GREEN (ready)
...
void led_service(void)
{
	...
	/* FLASH + factory blinks: TX only on phase edge. Leaving this family
	 * invalidates the solid-family cache below -- otherwise returning to a
	 * previously-shown solid color after a blink excursion would wrongly
	 * "skip transmit" and leave stale blink pixel data latched on the strip. */
	if (mode == LED_MODE_FLASH) {
